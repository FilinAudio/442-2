/* ============================================================
   FILIN LABS — GOLDEN SPEAKERS BATCH V16
   QUIET STABLE GRAND TOWER CARDINALITY over V14.

   Replaces V15 watchdog behavior:
   - does NOT load V15
   - no 500ms x 120 polling loop
   - no repeated console.info on every pass
   - one initial enforcement + debounced childList observer
   - logs only when the visible cardinality actually changes
   - preserves exactly one curation set and one promotions set
   ============================================================ */
(function(){
  'use strict';
  if(window.__FILIN_GOLDEN_SPEAKERS_BATCH_V16__) return;
  ...
  window.__FILIN_GOLDEN_SPEAKERS_BATCH_V16__=true;
  ...
  function load(u,id,done){
    ...
    s.onerror=function(){console.error('[Golden Speakers V16] failed',u);};
  ...
  function hide(el,reason){
    ...
    if(el.dataset.fpV16Hidden==='1')return false;
    el.dataset.fpV16Hidden='1';
    el.dataset.fpV16Reason=reason;
  ...
  function publish(extra,forceLog){
    ...
    var st={
      ...
      hiddenByV16:document.querySelectorAll('[data-fp-v16-hidden="1"]').length,
      passes:passes
    };
    if(extra)Object.keys(extra).forEach(function(k){st[k]=extra[k];});
    window.__FILIN_GOLDEN_SPEAKERS_BATCH_V16_STATE__=st;
    var sig=[st.goldenRoots,st.goldenCurationCards,st.goldenPromotionBlocks,st.visibleCurationSets,st.visiblePromotionSets,st.hiddenByV16].join('|');
    if(IS_GT && (forceLog||sig!==lastSignature)){
      lastSignature=sig;
      console.info('[Golden Speakers Batch V16] GRAND TOWER STABLE',st);
    }
    return st;
  }
  ...
  function start(){
    ...
    function schedule(){
      if(queued)return;
      queued=true;
      setTimeout(function(){queued=false;enforce(false);},180);
    }
    ...
    // Only two delayed safety passes for late Tilda/Swiper construction.
    setTimeout(function(){enforce(false);},1200);
    setTimeout(function(){enforce(false);},3500);
  }

  load(V14,'filin-golden-speakers-v14-from-v16',function(){
    start();
    console.info('[Golden Speakers Batch V16] READY',{version:'16.0.0',slug:slug,targeted:IS_GT});
  });
})();
