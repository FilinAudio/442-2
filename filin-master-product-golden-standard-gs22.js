/* ============================================================
   FILIN LABS — MASTER PRODUCT GOLDEN STANDARD GS2.2
   Freeze: 2026-08-20 / DAC bootstrap ordering fix

   Existing approved routes delegate to frozen GS2 unchanged.
   13 DAC / DAC+AMP routes use Batch 1.1 bootstrap, which guarantees
   #product-data.slug before Golden V3.3.2 is loaded. Batch V1 still
   gathers/verifies media BEFORE loading Golden core, so profile registration
   and explicit apply happen before core delayed callbacks can run.

   Gallery Integrity V3 remains the final mandatory sanitizer.
   ============================================================ */
(function(){
'use strict';
if(window.__FILIN_MASTER_PRODUCT_GOLDEN_STANDARD_GS22__)return;
window.__FILIN_MASTER_PRODUCT_GOLDEN_STANDARD_GS22__=true;
window.__FILIN_MASTER_PRODUCT_V3_GALLERY_THUMB_HEALTH_V1__=true;

var VERSION='GS-2026.08.20.2.2';
...
var state={version:VERSION,slug:PATH,group:isDac?'dacs-dac-amps':'gs2-existing',bootstrapLoaded:false,bootstrapReady:false,baseReady:false,integrityLoaded:false,integrityReady:false,ready:false,error:''};

function pub(){window.__FILIN_MASTER_PRODUCT_GOLDEN_STANDARD_GS22_STATE__=JSON.parse(JSON.stringify(state));}
...
async function boot(){
 try{
   if(!isDac){
     ...
     await wait(function(){var s=window.__FILIN_MASTER_PRODUCT_GOLDEN_STANDARD_GS2_STATE__;return s&&s.slug===PATH&&(s.ready===true||!!s.error);},58000);
 ...
 pub();
 if(state.error)console.warn('[Filin Labs Golden Standard GS2.2]',state.error,state);
 else if(state.ready)console.info('[Filin Labs Golden Standard GS2.2] ready',state);
}
