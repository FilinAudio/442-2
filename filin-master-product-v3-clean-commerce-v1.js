/* ============================================================
   FILIN LABS — MASTER PRODUCT V3 CLEAN COMMERCE V1
   Minimal production-safe bridge for clean V3 pages.
...
(function(){
  'use strict';
  if(window.__FILIN_MASTER_PRODUCT_V3_CLEAN_COMMERCE_V1__) return;
  window.__FILIN_MASTER_PRODUCT_V3_CLEAN_COMMERCE_V1__=true;
  ...
  function update(){
    ...
    var items=selected();
    var base=basePrice();
    var discountedAdd=items.reduce(function(sum,x){return sum+(x.price*0.95);},0);
    var total=Math.round((base+discountedAdd)*100)/100;
    var formatted=money(total);
    ...
    var state={
      ...
      selectedSubtotal:Math.round(items.reduce(function(s,x){return s+x.price;},0)*100)/100,
      selectedAfterDiscount:Math.round(discountedAdd*100)/100,
      total:total,
      items:items
    };
  ...
  console.info('[Master Product V3 Clean Commerce V1] READY');
})();
