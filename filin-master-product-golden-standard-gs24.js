/* ============================================================
   FILIN LABS — MASTER PRODUCT GOLDEN STANDARD GS2.4
   Freeze: 2026-08-20 / Required Contract for new migrated cards

   Existing approved routes delegate to frozen GS2.3 unchanged.
   ...
   4) Required Contract V1 guarantees:
      - 7 curation cards
      - Perfect Matches
      - black Handcrafted / curator strip
   5) Gallery Integrity V3 sanitizes and rebinds final media/interactions
   ============================================================ */
(function(){
'use strict';
if(window.__FILIN_MASTER_PRODUCT_GOLDEN_STANDARD_GS24__)return;
window.__FILIN_MASTER_PRODUCT_GOLDEN_STANDARD_GS24__=true;
window.__FILIN_MASTER_PRODUCT_V3_GALLERY_THUMB_HEALTH_V1__=true;

var VERSION='GS-2026.08.20.2.4';
...
var GS23='https://cdn.jsdelivr.net/gh/FilinAudio/442-2@203c0423cf626f4291dc0d19517be8c0be25b757/filin-master-product-golden-standard-gs23.js';
...
var CONTRACT='https://cdn.jsdelivr.net/gh/FilinAudio/442-2@6ba8b644ab40414fcef1d1d4341553da9c92e11c/filin-master-product-v3-required-contract-v1.js';
...
var state={
 version:VERSION,slug:PATH,group:isDac?'dacs-dac-amps':'gs23-existing',
 bootstrapLoaded:false,bootstrapReady:false,
 sanityLoaded:false,sanityReady:false,
 contractLoaded:false,contractReady:false,
 curationReady:false,pmReady:false,curatorReady:false,
 baseReady:false,integrityLoaded:false,integrityReady:false,
 ready:false,error:''
};

function pub(){window.__FILIN_MASTER_PRODUCT_GOLDEN_STANDARD_GS24_STATE__=JSON.parse(JSON.stringify(state));}
...
function visualReady(){
 var p=profile(),root=document.getElementById(ROOT);
 state.curationReady=!!(p&&Array.isArray(p.curation)&&p.curation.length===7&&root&&root.querySelectorAll('.v3-curation-item').length===7);
 state.pmReady=!!(root&&root.querySelector('.v3-pm'));
 state.curatorReady=!!arr(document.querySelectorAll('.fp-v3-curator-record')).find(function(x){var cs=getComputedStyle(x),r=x.getBoundingClientRect();return cs.display!=='none'&&cs.visibility!=='hidden'&&r.width>10&&r.height>10&&/handcrafted by/i.test(x.textContent||'');});
 return state.curationReady&&state.pmReady&&state.curatorReady;
}

async function boot(){
 try{
   if(!isDac){
     await load(GS23,function(){return !!window.__FILIN_MASTER_PRODUCT_GOLDEN_STANDARD_GS23__;});
     await wait(function(){var s=window.__FILIN_MASTER_PRODUCT_GOLDEN_STANDARD_GS23_STATE__;return s&&s.slug===PATH&&(s.ready===true||!!s.error);},58000);
     var old=window.__FILIN_MASTER_PRODUCT_GOLDEN_STANDARD_GS23_STATE__;
   ...
   state.baseReady=rootReady()&&!!profile();
   ...
   await load(CONTRACT,function(){return !!window.__FILIN_MASTER_PRODUCT_V3_REQUIRED_CONTRACT_V1__;});
   state.contractLoaded=true;pub();
   state.contractReady=await wait(function(){var s=window.__FILIN_MASTER_PRODUCT_V3_REQUIRED_CONTRACT_V1_STATE__;return s&&s.slug===PATH&&(s.ready===true||!!s.error);},26000);
   var cs=window.__FILIN_MASTER_PRODUCT_V3_REQUIRED_CONTRACT_V1_STATE__;
   if(!cs||cs.ready!==true)throw new Error('Required Golden contract did not become ready: '+(cs&&cs.error||'timeout'));

   state.baseReady=rootReady()&&!!profile();
   if(!state.baseReady)throw new Error('Golden root/profile missing after required contract');
   if(!visualReady())throw new Error('Required visual contract incomplete after post-build pass');
   ...
   await wait(function(){return visualReady();},8000);
   if(!visualReady())throw new Error('Required visual contract lost after gallery integrity pass');

   state.ready=true;state.error='';
 }catch(e){state.error=String(e&&e.message||e);state.ready=false;}
 pub();
 if(state.error)console.warn('[Filin Labs Golden Standard GS2.4]',state.error,state);
 else if(state.ready)console.info('[Filin Labs Golden Standard GS2.4] ready',state);
}
