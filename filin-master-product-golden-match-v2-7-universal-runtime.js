/* FILIN LABS — MASTER PRODUCT GOLDEN MATCH V2.7 UNIVERSAL
...
(function () {
  ...
  if (window.__FILIN_MASTER_GOLDEN_MATCH_V27__) return;
  window.__FILIN_MASTER_GOLDEN_MATCH_V27__ = true;
  window.__FILIN_MASTER_GOLDEN_MATCH_V24__ = true; // compatibility guard

  var STYLE_ID = 'filin-master-golden-match-v27';
  ...
  function isTarget() {
    var slug = String(readSeed().slug || '');
    return slug === 'filin_audio_quadron' ||
      slug === 'audioinstrument_grand_tower_speakers';
  }
  ...
  function currentSlug() {
    return String(readSeed().slug || '').trim();
  }

  function currentProfile() {
    try {
      var api = window.FilinFullProductProfilesV27 || window.FilinFullProductProfilesV2;
      return api && typeof api.get === 'function' ? api.get(currentSlug()) : null;
    } catch (e) {
      return null;
    }
  }
  ...
  function currentProductLabel() {
    ...
    if (p && p.reviewsQuery) return String(p.reviewsQuery);
    var seed = readSeed();
    return norm([seed.brand || '', seed.model || ''].join(' ')) ||
      norm(seed.name || '') ||
      'this product';
  }

  function currentReviewIntro() {
    var p = currentProfile();
    if (p && p.reviewsIntro) return String(p.reviewsIntro);
    return 'Share your listening experience with ' + currentProductLabel() + '.';
  }
  ...
  function injectStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      html[data-filin-full-profile] body,
      html[data-filin-full-profile] .js-product,
      html[data-filin-full-profile] .js-product *,
      html[data-filin-full-profile] .fp-product-overview,
      html[data-filin-full-profile] .fp-product-overview *,
      html[data-filin-full-profile] .fp-curation,
      html[data-filin-full-profile] .fp-curation * {
...
.js-product .tabs-wrapper {
  ...
  function activeRoot() {
    ...
    var exact = slug
      ? document.querySelector('.js-product[data-fp-full-profile="' + CSS.escape(slug) + '"]')
      : null;
    if (exact) return exact;

    var profiled = document.querySelector('.js-product[data-fp-full-profile]');
    if (profiled) return profiled;

    var roots = Array.prototype.slice.call(document.querySelectorAll('.js-product'));
    var seed = readSeed();
    var model = norm(seed.model || '');
    if (model) {
      var hit = roots.find(function (root) {
        var name = root.querySelector('#tilda-product-name,.js-product-name');
        return norm(name && name.textContent).toLowerCase().indexOf(model.toLowerCase()) !== -1;
      });
      if (hit) return hit;
    }
    return roots[0] || null;
  ...
  function markHero() {
    var title = document.querySelector('.t-cover .t184__title, .t184__title, h1');
  ...
  function shortenStickyTitle() {
    ...
    var seed = readSeed();
    var model = norm(seed.model || '');
    var pinfo = currentProfile();
    var desired = pinfo && pinfo.commerce && pinfo.commerce.stickyTitle
      ? String(pinfo.commerce.stickyTitle)
      : (norm([seed.brand || '', seed.model || ''].join(' ')) || norm(seed.name || ''));

    if (!model || !desired) return 0;

    Array.prototype.slice.call(document.querySelectorAll('body *')).forEach(function (el) {
      if (el.children.length) return;

      var t = norm(el.textContent);
      if (!t || t.toLowerCase().indexOf(model.toLowerCase()) === -1) return;
  ...
  function migrateReviews() {
    ...
    /* Search the WHOLE canonical tabs wrapper, not only the desktop header.
       On mobile the real buttons live inside .fp-v26-real-accordion; searching
       only targetHeader caused a new Reviews button to be created on every retry. */
    var button = Array.prototype.slice.call(targetTabs.querySelectorAll('.tab-btn'))
      .find(function (btn) {
        return /^Reviews\b/i.test(norm(btn.textContent));
      });
    ...
    reviewContent.querySelectorAll('.other-reviews-btn').forEach(function (a) {
      var reviewName = currentProductLabel();
      a.setAttribute('href','/insights?product=' + encodeURIComponent(reviewName));
      a.setAttribute('aria-label','Open other reviews for ' + reviewName);
    });
    ...
    if (intro) {
      intro.textContent = currentReviewIntro();
    }
...
function hideLegacyGrandTower() {
  if (currentSlug() !== 'filin_audio_quadron') return 0;
  var root = activeRoot();
  var hidden = 0;
  ...
  function apply() {
    ...
    document.documentElement.setAttribute('data-fp-golden-match','v2.7');

    console.info(
      '[Master Product V2] GOLDEN MATCH V2.7 UNIVERSAL APPLIED',
      result
    );
  }

  window.FilinMasterGoldenMatchV27Universal = Object.freeze({
    version:'2.7',
    apply:apply
  });
...
(function(){
  if(!document.getElementById('filin-master-pm-v27-style')){
    var s=document.createElement('style');
    s.id='filin-master-pm-v27-style';
    s.textContent=`/* =========================================================
...
(function () {
  ...
  if (window.__MASTER_PRODUCT_PM_V27__) return;
  window.__MASTER_PRODUCT_PM_V27__ = true;

  var ROLE_RULES = [
    { test: /quadron/i, role: 'Core planar headphones' },
    { test: /grand tower/i, role: 'Core loudspeakers' },
    { test: /tinker|network player/i, role: 'Network transport' },
    { test: /solero|network switch/i, role: 'Network conditioning' },
    { test: /otis|dsd1794|pcm1794/i, role: 'Digital-to-analog conversion' },
    { test: /sirius|kt150/i, role: 'Tube amplification' },
    { test: /a-1|speaker cables/i, role: 'Speaker cable' },
...
setTimeout(function(){
  if (window.FilinMasterGoldenMatchV27Universal) {
    window.FilinMasterGoldenMatchV27Universal.apply();
  }
}, 3700);
...
(function () {
  ...
  if (window.__FILIN_MASTER_GOLDEN_MATCH_V27_FINALIZER__) return;
  window.__FILIN_MASTER_GOLDEN_MATCH_V27_FINALIZER__ = true;

  var MAX = 820;
  var STYLE_ID = 'filin-master-golden-match-v27-final';
  ...
  function currentSlugV27() {
    try {
      var el = document.getElementById('product-data');
      var seed = el ? JSON.parse(el.textContent || '{}') : {};
      return String(seed.slug || '').trim();
    } catch (e) {
      return '';
    }
  }
  ...
  function root() {
    var slug = currentSlugV27();
    if (slug) {
      var exact = document.querySelector('.js-product[data-fp-full-profile="' + CSS.escape(slug) + '"]');
      if (exact) return exact;
    }
    return document.querySelector('.js-product[data-fp-full-profile]') ||
      document.querySelector('.js-product') || null;
  }
  ...
  function ensureHero() {
    ...
    var descText = p && p.hero && p.hero.description
      ? p.hero.description
      : norm(hero.querySelector('.t184__descr') && hero.querySelector('.t184__descr').textContent);
  ...
  function restoreDesktopTabs(r, wrapper, header) {
    ...
    buttons.forEach(function (btn) {
      ...
      cleanBtn.classList.remove('fp-v26-real-acc-btn','fl-acc-open');
      cleanBtn.removeAttribute('aria-expanded');
      cleanBtn.removeAttribute('data-fp-v26-bound');
      cleanBtn.removeAttribute('data-fp-v262-desktop-bound');
      var arrow = cleanBtn.querySelector('.fp-v26-real-arrow');
  ...
  function ensureAccordion() {
    ...
    /* Safety net: a previous cached/runtime retry may have already created
       duplicate Reviews buttons. Keep exactly one before deciding whether the
       accordion needs rebuilding. */
    var reviewButtons = Array.prototype.slice.call(wrapper.querySelectorAll('.tab-btn'))
      .filter(function (btn) { return /^Reviews\b/i.test(norm(btn.textContent)); });
    reviewButtons.slice(1).forEach(function (btn) { btn.remove(); });
    ...
    if (existing) {
      Array.prototype.slice.call(existing.children).forEach(function (node) {
        if (node.classList.contains('fp-v26-real-acc-btn')) {
          /* Clone instead of moving the old button. The old mobile listener
             closes over the previous accordion node and would otherwise
             survive a rebuild, call stopImmediatePropagation(), and make the
             newly rebuilt tabs look dead. */
          var cleanBtn = node.cloneNode(true);
          cleanBtn.classList.remove('fp-v26-real-acc-btn','fl-acc-open');
          cleanBtn.removeAttribute('data-fp-v26-bound');
          cleanBtn.removeAttribute('data-fp-v262-desktop-bound');
          cleanBtn.removeAttribute('aria-expanded');
          var cleanArrow = cleanBtn.querySelector('.fp-v26-real-arrow');
          if (cleanArrow) cleanArrow.remove();
          header.appendChild(cleanBtn);
          node.remove();
  ...
  function hydratePromotionsFromCatalog(promotions) {
    ...
    var usable = catalog.filter(function (product) {
      ...
      if (String(product.slug || '') === currentSlugV27()) return false;
  ...
  function finalize() {
    ...
    document.documentElement.setAttribute('data-fp-golden-final','v2.7');

    console.info('[Master Product V2] GOLDEN MATCH V2.7 UNIVERSAL FINALIZED',result);
    return result;
  }

  window.FilinMasterGoldenMatchV27Final = Object.freeze({
    version:'2.7',
    apply:finalize
  });
...
(function () {
  ...
  if (window.__FILIN_MASTER_GOLDEN_MATCH_V27_DESKTOP_FINISHER__) return;
  window.__FILIN_MASTER_GOLDEN_MATCH_V27_DESKTOP_FINISHER__ = true;

  var STYLE_ID = 'filin-master-golden-match-v27-desktop';
  ...
  function refreshPromotions() {
    try {
      if (window.FilinMasterGoldenMatchV27Final &&
          typeof window.FilinMasterGoldenMatchV27Final.apply === 'function') {
        window.FilinMasterGoldenMatchV27Final.apply();
      }
    } catch (e) {
  ...
  function apply() {
    ...
    document.documentElement.setAttribute('data-fp-golden-desktop','v2.7');

    console.info('[Master Product V2] GOLDEN MATCH V2.7 UNIVERSAL DESKTOP FINISHER APPLIED', {
  ...
  window.FilinMasterGoldenMatchV27Desktop = Object.freeze({
    version:'2.7',
    apply:apply
  });
})();
...
(function () {
  ...
  if (window.__FILIN_MASTER_GOLDEN_MATCH_V27_DESKTOP_INTERACTION__) return;
  window.__FILIN_MASTER_GOLDEN_MATCH_V27_DESKTOP_INTERACTION__ = true;

  var MAX = 820;
  var STYLE_ID = 'filin-master-golden-match-v27-desktop-interaction';
  ...
  function apply() {
    ...
    console.info('[Master Product V2] GOLDEN MATCH V2.7 UNIVERSAL DESKTOP APPLIED', {
      tabs:tabs,
  ...
  window.FilinMasterGoldenMatchV27DesktopInteraction = Object.freeze({
    version:'2.7',
    apply:apply
  });
})();
...
/* ============================================================
   FILIN LABS — GOLDEN MATCH V2.6.4
   MOBILE HERO SIZE + TAB STABILITY SAFE OVERRIDE
   Base: exact working V2.6.2 before hero regression.
   IMPORTANT: no commerce / tabs / reviews / PM / promotions DOM changes.
   ============================================================ */
(function () {
  'use strict';

  var STYLE_ID = 'filin-golden-v27-mobile-hero';

  function installHeroOnlyOverride() {
    var old = document.getElementById(STYLE_ID);
    if (old) old.remove();
  ...
  installHeroOnlyOverride();

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', installHeroOnlyOverride, { once:true });
  }

  /* Tilda / runtime retries may append styles later. Re-append ONLY this CSS override. */
  setTimeout(installHeroOnlyOverride, 700);
  setTimeout(installHeroOnlyOverride, 1800);
  setTimeout(installHeroOnlyOverride, 3400);

  console.info('[Master Product V2] GOLDEN MATCH V2.7 UNIVERSAL MOBILE HERO APPLIED');
})();


/* ============================================================
   FILIN LABS — GOLDEN MATCH V2.7 UNIVERSAL STABILITY FINISHER
   - desktop open Perfect Matches header becomes content-width so the
     chevron sits next to the "device." copy instead of the far right edge
   - adds breathing room below Ultimate Synergy
   - visual-only; no product/commerce DOM mutation
   ============================================================ */
(function () {
  'use strict';

  var STYLE_ID = 'filin-golden-v27-stability-finisher';
  ...
  function install() {
    ...
    style.textContent = `
      @media (min-width:821px) {
        .js-product .perfect-matches-block.fp-pm-v5.fp-pm-open > .fp-pm-v5-toggle {
        ...
        .js-product .perfect-matches-block.fp-pm-v5.fp-pm-open > .fp-pm-v5-toggle .fp-pm-v5-toggle-copy {
          width:auto !important;
          max-width:none !important;
        }

        .js-product .perfect-matches-block.fp-pm-v5.fp-pm-open {
  ...
  console.info('[Master Product V2] GOLDEN MATCH V2.7 UNIVERSAL STABILITY FINISHER APPLIED');
})();
