/* FILIN LABS — MASTER PRODUCT FULL PROFILE V2.1
   Quadron control profile built from the production page DOM capture.
...
(function() {
  ...
  function readSeed() {
    ...
    try {
      ...
      console.error('[Master Product V2] Full Profile: invalid #product-data', err);
      return null;
    }
  }
  ...
  function currentSlug() {
    const seed = readSeed();
    return String((seed && seed.slug) || '').trim();
  }

  function getProfile(slug) {
    return profiles[String(slug || '').trim()] || null;
  }

  function patchSeed(profile) {
    const el = document.getElementById('product-data');
    if (!el) return false;

    const seed = readSeed() || {};
    seed.id = profile.id;
    seed.slug = profile.slug;
    seed.name = profile.commerce.cartName;
    seed.category = profile.category;
    seed.currency = profile.currency;
    seed.price = profile.commerce.basePrice;
    seed.url = 'https://filinlabs.com/' + profile.slug;
    seed.stickyTitle = profile.commerce.stickyTitle;
  ...
  function findHero() {
    const h1 = document.querySelector('h1.t184__title, h1');
    if (!h1) return null;
    const rec = h1.closest('.t-rec,[id^="rec"]') || h1.parentElement;
    return { h1, rec };
  }

  function bindHero(profile) {
    const hero = findHero();
    if (!hero) return { found:false };

    const descr = hero.rec.querySelector('.t184__descr');
    if (descr) descr.textContent = profile.hero.description;

    const carrier =
      hero.rec.querySelector('.t-cover__carrier') ||
      hero.rec.querySelector('[id^="coverCarry"]');

    if (carrier) {
      const image = profile.hero.background;
      carrier.style.backgroundImage = 'url("' + image.replace(/"/g, '\\"') + '")';
      carrier.setAttribute('data-content-cover-bg', image);
      carrier.setAttribute('data-original', image);
      const meta = carrier.querySelector('meta[itemprop="image"]');
      if (meta) meta.setAttribute('content', image);
    }

    const actual = (hero.h1.innerText || '').replace(/\s+/g,' ').trim();
    const expected = profile.hero.staticH1.replace(/\s+/g,' ').trim();
    if (actual !== expected) {
      console.warn(
        '[Master Product V2] STATIC H1 MISMATCH — change it in Tilda for SEO.',
        { actual, expected }
      );
    }

    return { found:true, description:!!descr, background:!!carrier, h1Matches:actual===expected };
  }

  function bindCurator(profile) {
    const candidates = Array.from(document.querySelectorAll('body *'))
      .filter(el => {
        const text = (el.innerText || '').replace(/\s+/g,' ').trim();
        return /^Handcrafted by/i.test(text) && text.length < 350;
      })
      .sort((a,b) => (a.innerText || '').length - (b.innerText || '').length);

    const el = candidates[0];
    if (!el) return false;
    el.textContent = profile.curator;
    return true;
  }

  function findOverviewGallery(overview) {
    const overviewElem = overview && overview.closest('.t396__elem');
    const artboard = overviewElem && overviewElem.closest('.t396__artboard');
    ...
    if (artboard) {
      const local = Array.from(
        artboard.querySelectorAll('.t396__elem[data-elem-type="gallery"]')
      ).filter(usable);
    ...
    const all = Array.from(
      document.querySelectorAll('.t396__elem[data-elem-type="gallery"]')
    ).filter(usable);
  ...
  function bindOverview(profile) {
    const overview = document.querySelector('.fp-product-overview,[data-fp-product-overview]');
    if (!overview) return { overview:false, gallery:0 };

    const title = overview.querySelector('h2');
    if (title) title.textContent = profile.overview.title;

    Array.from(overview.querySelectorAll('p,h3')).forEach(el => el.remove());

    const temp = document.createElement('div');
    temp.innerHTML = profile.overview.html;

    const wishlist = overview.querySelector('.fp-native-wishlist-link');
    Array.from(temp.childNodes).forEach(node => {
      if (wishlist) overview.insertBefore(node, wishlist);
      else overview.appendChild(node);
    });

    const gallery = findOverviewGallery(overview);
    let count = 0;

    if (gallery) {
      const imgs = profile.overview.galleryImages;
      const bgNodes = Array.from(gallery.querySelectorAll('.tn-atom__slds-img'));

      bgNodes.forEach((el, index) => {
        const image = imgs[index % imgs.length];
        if (!image) return;
        el.style.backgroundImage = 'url("' + image.replace(/"/g, '\\"') + '")';
        el.setAttribute('data-original', image);
        el.setAttribute('data-img-zoom-url', image);
        el.setAttribute('data-bg', image);
        count++;
      });
      ...
      Array.from(gallery.querySelectorAll('img')).forEach((img,index) => {
        ...
        img.alt = profile.overview.title;
      });
    }

    return { overview:true, gallery:count };
  }

  function bindCuration(profile) {
    const root = document.querySelector('.fp-curation');
    if (!root) return 0;

    const items = Array.from(root.querySelectorAll('.fp-curation-item'));
    if (!items.length) return 0;

    profile.curation.forEach((data,index) => {
      const item = items[index];
      if (!item) return;
      const h = item.querySelector('h3');
      const p = item.querySelector('p');
      if (h) h.textContent = data.title;
      if (p) p.innerHTML = data.html;
    });

    // Hide any extra template items if the Master has more than the profile.
    items.slice(profile.curation.length).forEach(el => el.style.display='none');
    return Math.min(items.length, profile.curation.length);
  }
  ...
  function installTabs(root) {
    ...
    buttons.forEach(btn => {
      ...
      const match = inline.match(/showTab\(event,\s*['"](.+?)['"]\)/);
      if (match) btn.dataset.fpTarget = match[1];
  ...
  function installOptionPriceLogic(root, profile) {
    if (!root || root.dataset.fpPriceLogic === '1') return;
    root.dataset.fpPriceLogic = '1';

    const basePrice = Number(profile.commerce.basePrice) || 0;
    const priceEl = root.querySelector('#main-price') || document.getElementById('main-price');
    const nameEl = root.querySelector('#tilda-product-name') || document.getElementById('tilda-product-name');
    const discountMsg = root.querySelector('#bundle-discount-msg');
    const sliderWrapper = root.querySelector('#slider-wrapper');
    const lengthSlider = root.querySelector('#length-slider');
    const lengthDisplay = root.querySelector('#length-display');
    const lengthPriceDisplay = root.querySelector('#length-price-display');

    function update() {
      let total = 0;
      const selected = [];

      root.querySelectorAll('.price-item:checked').forEach(input => {
        total += parseInt(input.getAttribute('data-price'),10) || 0;
        const label = input.closest('label');
        const text = label ? label.textContent.replace(/\s+/g,' ').trim() : '';
        if (text && text !== 'Stock: No options' && !text.startsWith('Stock:')) selected.push(text);
      });

      const cableType = root.querySelector('input[name="cable-type"]:checked');
      if (cableType && sliderWrapper) {
        if (cableType.value === 'custom') {
          sliderWrapper.style.display = 'block';
          if (lengthSlider && lengthDisplay) {
            const length = parseFloat(lengthSlider.value || '2');
            lengthDisplay.textContent = length.toFixed(1);
            if (lengthPriceDisplay) lengthPriceDisplay.textContent = '0';
            if (length > 2) selected.push('Custom Cable Length: ' + length.toFixed(1) + 'm');
          }
        } else {
      ...
      if (priceEl) priceEl.textContent = String(basePrice + total);

      if (nameEl) {
        nameEl.textContent = selected.length
          ? profile.commerce.displayName + ' [' + selected.join(' | ') + ']'
          : profile.commerce.cartName;
      }

      const bundleItems = Array.from(root.querySelectorAll('.bundle-item'));
      const allBundle = bundleItems.length > 0 && bundleItems.every(x => x.checked);
      if (discountMsg) {
        discountMsg.innerHTML = allBundle
          ? '<b>5% Bundle Discount</b> for Each Added item was successfully applied to total!'
          : 'Add recommended synergy components to get <b>5% OFF</b> per each item added.';
      }

      document.dispatchEvent(new CustomEvent('filin:product-profile-price-update', {
        detail: {
          slug: profile.slug,
          price: basePrice + total,
          name: nameEl ? nameEl.textContent : profile.commerce.cartName
        }
      }));
    }

    root.addEventListener('change', evt => {
      if (evt.target.matches('.price-item,input[name="cable-type"]')) update();
    });
  ...
  function bindCommerce(profile) {
    const root = document.querySelector('.js-product');
    if (!root) return false;

    if (root.dataset.fpFullProfile !== profile.slug) {
      root.innerHTML = profile.commerce.innerHTML;
      root.dataset.fpFullProfile = profile.slug;
    }

    installTabs(root);
    installOptionPriceLogic(root, profile);
    return true;
  }

  function bindReviews(profile) {
    const candidates = Array.from(document.querySelectorAll('a,button'))
      .filter(el => /View The Reviews|Other Reviews/i.test((el.innerText || '').trim()));

    if (!candidates.length) return false;
    candidates.forEach(el => el.textContent = profile.reviewsCTA);
    return true;
  }

  function bindSticky(profile) {
    let changed = 0;

    /*
      Product Sticky Header V4 has a stable public DOM contract.
      Use it directly instead of relying on text-pattern scanning.
    */
    const direct =
      document.querySelector(
        '#fl-product-sticky-header .flph-product-name'
      );

    if (direct) {
      const wanted = profile.commerce.stickyTitle;
      if ((direct.textContent || '').trim() !== wanted) {
        direct.textContent = wanted;
        changed++;
      }
    }

    /*
      Compatibility fallback for an older/alternate sticky header.
    */
    const patterns = [
      /Filin Audio Quadron Bespoke High-End Planar Magnetic Headphones/i,
      /Filin Audio "Quadron" Planar Magnetic Headphones/i,
      /Filin Audio Model 1 Standard V2/i,
      /Audioinstrument.*Grand Tower/i
    ];

    Array.from(document.querySelectorAll('body *')).forEach(el => {
      if (el === direct || el.children.length) return;
      ...
      if (sticky) {
        el.textContent = profile.commerce.stickyTitle;
        changed++;
      }
    });
  ...
  function apply() {
    ...
    if (!profile) {
      console.warn('[Master Product V2] Full Profile not found for slug:', slug);
      return false;
    }
    ...
    const result = {
      slug,
      hero: bindHero(profile),
      curator: bindCurator(profile),
      overview: bindOverview(profile),
      curationItems: bindCuration(profile),
      commerce: bindCommerce(profile),
      reviews: bindReviews(profile),
      sticky: bindSticky(profile)
    };

    document.documentElement.setAttribute('data-filin-full-profile', slug);

    console.info('[Master Product V2] FULL PRODUCT PROFILE V2.2 APPLIED', result);
    return true;
  }
  ...
  window.FilinFullProductProfilesV2 = Object.freeze(api);
  ...
  // Bounded retries: Tilda mounts some blocks later.
