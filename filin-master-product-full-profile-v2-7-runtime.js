/* FILIN LABS — MASTER PRODUCT FULL PROFILE V2.7
   Universal control profile.
   Profiles included:
   - Filin Audio Quadron
   - Audioinstrument Grand Tower
*/
(function() {
  'use strict';

  if (window.__FILIN_MASTER_FULL_PROFILE_V27__) return;
  window.__FILIN_MASTER_FULL_PROFILE_V27__ = true;
  ...
  function bindHero(profile) {
    ...
    if (carrier) {
      const image = profile.hero && profile.hero.background
        ? String(profile.hero.background).trim()
        : '';
      if (image) {
  ...
  function apply() {
    ...
    console.info('[Master Product V2] FULL PRODUCT PROFILE V2.7 APPLIED', result);
    return true;
  }
  ...
  window.FilinFullProductProfilesV27 = frozenApi;
  window.FilinFullProductProfilesV2 = frozenApi; // backward-compatible alias for Golden Match layers.
