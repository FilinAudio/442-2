...
function setSticky(total){Array.prototype.slice.call(document.querySelectorAll('body *')).forEach(function(el){if(el.children.length)return;var t=(el.textContent||'').trim();if(!/^\$[\d,]+(?:\.\d+)?$/.test(t))return;var p=el;for(var i=0;i<6&&p;i++,p=p.parentElement){var pos=getComputedStyle(p).position;if(pos==='fixed'||pos==='sticky'){el.textContent=money(total);break}}})}
function update(){var r=root();if(!r)return false;var items=selected();var add=items.reduce(function(s,x){return s+x.price*.95},0);var total=basePrice()+add;var bp=r.querySelector('.v3-buy-price');if(bp)bp.textContent=money(total);var np=r.querySelector('#v3-main-price');if(np)np.textContent=String(Math.round(total*100)/100);setSticky(total);window.__FILIN_GOLDEN_SPEAKERS_BATCH_V7_STATE__={slug:slug,selected:items.length,total:total,items:items};return true}
...
function bind(){css();document.addEventListener('change',function(e){if(e.target&&e.target.matches('#filin-master-product-v3 .v3-pm .v3-bundle'))setTimeout(update,0)},true);var mo=new MutationObserver(function(){css();setTimeout(update,0)});mo.observe(document.documentElement,{childList:true,subtree:true});setTimeout(update,1000)}
