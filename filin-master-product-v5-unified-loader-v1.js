/* ============================================================
   FILIN LABS — MASTER PRODUCT V5 UNIFIED LOADER V1
   One global entrypoint for approved Golden product pages.
   Fixes old-Tilda flash / repeated route loading across:
   - Golden Reference 2 headphone/product batch
   - Audioinstrument Grand Tower speakers
   - approved 10 tube-amplifier pages

   AMP route intentionally SKIPS V4.4/V5/V6/V7 Zero-Block wrappers.
   Only V4.3 -> V4.2 -> V4.1 is allowed until static AMP profiles replace it.
   ============================================================ */
(function(){
'use strict';
if(window.__FILIN_MASTER_PRODUCT_V5_UNIFIED_LOADER_V1__)return;
...
'filin_audio_limited','snorry_trion_mk_3'
];
var CORE_STATIC=['filin_audio_quadron','audioinstrument_grand_tower_speakers'];
var AMPS=[
...
var all=GR2.concat(CORE_STATIC,AMPS);if(all.indexOf(PATH)<0)return;
window.__FILIN_MASTER_PRODUCT_V5_UNIFIED_LOADER_V1__=true;

/* Prevent old AMP wrappers placed later in HEAD from starting again. */
...
var state={version:VERSION,slug:PATH,group:'',ready:false,root:false,released:false,timeout:false,error:''};
function pub(){window.__FILIN_MASTER_PRODUCT_V5_UNIFIED_LOADER_V1_STATE__=JSON.parse(JSON.stringify(state));}
function arr(v){return Array.prototype.slice.call(v||[])}
function has(xs,x){return xs.indexOf(x)>=0}
...
function load(src,test){return new Promise(function(resolve,reject){
  ...
  if(loaded(src)){var n=0,t=setInterval(function(){if(!test||test()){clearInterval(t);resolve(true);}else if(++n>120){clearInterval(t);resolve(false);}},50);return;}
...
/* PRE-PAINT CURTAIN: old Tilda product must never be visible. */
var style=document.createElement('style');style.id='filin-v5-unified-preboot-style';
style.textContent='html.fp-v5-product-boot body{visibility:hidden!important}html.fp-v5-product-boot:after{content:"FILIN LABS";position:fixed;inset:0;z-index:2147483646;display:flex;align-items:center;justify-content:center;background:#f8f5f1;color:#2d241b;font-family:Montserrat,Arial,sans-serif;font-size:13px;font-weight:700;letter-spacing:.18em;visibility:visible!important;opacity:1!important}';
(document.head||document.documentElement).appendChild(style);document.documentElement.classList.add('fp-v5-product-boot');
function release(){if(state.released)return;state.released=true;document.documentElement.classList.remove('fp-v5-product-boot');var s=document.getElementById('filin-v5-unified-preboot-style');if(s)s.remove();pub();}
function waitRoot(ms){return new Promise(function(resolve){var st=Date.now(),t=setInterval(function(){var r=document.getElementById(ROOT);if(r&&r.querySelector('.v3-shell')){clearInterval(t);resolve(r);}else if(Date.now()-st>(ms||9000)){clearInterval(t);resolve(null);}},40);});}
...
var DEP={
 ...
 amp:CDN+'6904666697792cbb30d7e46bf50b00d5ffe9580b/filin-master-product-v4-production-amp-batch-v3.js',
...
/* correct V4.3 commit used in production */
DEP.amp='https://cdn.jsdelivr.net/gh/FilinAudio/442-2@5a656ecf9e6f9df16b35dcdf284012b7773939b3/filin-master-product-v4-production-amp-batch-v3.js';

async function coreStatic(){
 await load(DEP.core,function(){return !!window.FilinMasterProductV3;});
 await load(DEP.registry,function(){return !!window.__FILIN_MASTER_PRODUCT_V3_PROFILES_REGISTRY_V1__;});
 try{window.FilinMasterProductV3&&window.FilinMasterProductV3.apply&&window.FilinMasterProductV3.apply();}catch(e){}
...
async function boot(){
 try{
   if(has(AMPS,PATH)){
     ...
     /* Explicitly skip V4.4+; V4.3 owns PM and loads only V4.2 -> V4.1. */
     await load(DEP.amp,function(){return !!window.__FILIN_MASTER_PRODUCT_V4_AMP_BATCH_V3__;});
   }else if(has(CORE_STATIC,PATH)){
     state.group='core-static';pub();await coreStatic();
   }else{
     state.group='golden-reference-2';pub();
     await load(DEP.gr2,function(){return !!window.__FILIN_GR2_BATCH_V3__;});
   }
   var root=await waitRoot(9000);state.root=!!root;state.ready=!!root;
   if(!root){state.timeout=true;state.error='Golden root timeout';}
 }catch(e){state.error=String(e&&e.message||e);}
 pub();requestAnimationFrame(function(){requestAnimationFrame(release);});
 if(state.error)console.warn('[Filin V5 Unified]',state.error);else console.info('[Filin Labs] Master Product V5 Unified ready',{version:VERSION,slug:PATH,group:state.group});
}

/* Never leave the visitor on a blank screen if an external CDN fails. */
setTimeout(function(){if(!state.released){state.timeout=true;state.error=state.error||'failsafe release';release();}},11000);
