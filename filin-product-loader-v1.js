/*! filin-product-loader v1.1.4 — параллельный стек, мягкий откат */
(function () {
  'use strict';
  ...
  function has(src) {
    for (var i = 0; i < document.scripts.length; i++) {
      if (
        ...
        document.scripts[i].src.split('?')[0] === src.split('?')[0]
      ) {
        return true;
      }
    }
  ...
  function restore(why) {
    ...
    console.warn(
      '[Filin Loader] карточка не собрана (' +
      why +
      ') — легаси-блоки возвращены'
    );
  }
  ...
  function boot(routes) {
    ...
    if (((routes && routes.slugs) || []).indexOf(s) < 0) {
      log('не карточка товара:', s || '/');
      return;
    }
    ...
    Promise.all([
      ...
      if (!api || !api.profiles) {
        throw new Error('ядро не инициализировалось');
      }

      if (!p) {
        throw new Error('профиль пуст');
      }
      ...
      log(
        'готово:',
        s,
        p.overview.galleryImages.length + ' изображений'
      );
