...
#
# FILIN LABS — v1.1.2
#   • картинки Zero-блоков: сканируем HTML записи регуляркой
#     (они в style="background-image:url(...)" на tn-atom, атрибутов нет)
#   • слипшийся текст курации: границы блоков превращаются в пробел
#
# Запуск:  GITHUB_TOKEN=ghp_… bash deploy-v1.1.2.sh
#

set -e
[ -z "$GITHUB_TOKEN" ] && { echo "Задайте GITHUB_TOKEN=ghp_… перед командой"; exit 1; }
...
GIT_LFS_SKIP_SMUDGE=1 git clone -q --depth 1 "https://${GITHUB_TOKEN}@github.com/FilinAudio/442-2.git" repo || {
  echo "  обрыв сети — запустите скрипт ещё раз"; exit 1; }
cd repo
git config user.email "build@filinlabs.com"
git config user.name  "Filin Build Agent"
...
node tools/extract-profiles.mjs 2>&1 | tail -50

echo "[6/6] Коммит и тег v1.1.2…"
printf 'node_modules/\n' > .gitignore
git rm -r --cached node_modules -q 2>/dev/null || true
git add -A
git commit -q -m "v1.1.2: scan record HTML for Zero Block images; fix run-together curation text"
git tag -f v1.1.2 -m v1.1.2 >/dev/null
git push -q origin HEAD:main
git push -q -f origin v1.1.2
...
echo ""
cat <<'SNIP'
<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/FilinAudio/442-2@v1.1.2/generated/legacy-hide.css">
<script defer src="https://cdn.jsdelivr.net/gh/FilinAudio/442-2@v1.1.2/filin-product-loader-v1.js"></script>
<script defer src="https://cdn.jsdelivr.net/gh/FilinAudio/442-2@v1.1.2/generated/filin-routes.js"></script>
SNIP
echo "════════════════════════════════════════════════════"
