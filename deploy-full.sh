...
#
# Запуск:  GITHUB_TOKEN=your_token_here bash deploy-v1.1.0.sh
#

set -e

if [ -z "$GITHUB_TOKEN" ]; then
  echo "Ошибка: Переменная GITHUB_TOKEN не задана."
  echo "Запуск: GITHUB_TOKEN=ghp_your_token bash deploy-v1.1.0.sh"
  exit 1
fi
...
echo "[1/6] Клонирую (без LFS-файлов и истории)…"
