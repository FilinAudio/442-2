...
if [[ -z "$DRY" ]]; then
  ...
  git tag -f v1.0.0 --quiet
  git push origin main --force --quiet 2>&1 | grep -v "^To https"
  git push origin v1.0.0 --force --quiet 2>&1 | grep -v "^To https"
  echo "✓ Push завершён"
else
  echo ""
