...
/* =========================================================
   FILIN MASTER PRODUCT V3 PROFILE — SIRIUS KT150
   V1.1 LEGACY GALLERY HYDRATOR
   Restores the complete legacy Tilda gallery without touching
   the frozen Golden Standard core. This is profile/data logic only.
   ========================================================= */
(function(){
  'use strict';

  var SLUG='audioinstrument_sirius_kt150_tube_amplifier';
  var LEGACY_URL='/audioinstrument_sirius_kt150_tube_amplifier';
  var LEGACY_GALLERY_REC='rec2355175041';
  var started=false;
  ...
  function cleanUrl(raw){
    ...
    try { s=new URL(s, location.origin).href; } catch(e) { return ''; }
    return s;
  }

  function looksLikeImage(u){
    if(!u) return false;
    if(u.indexOf('static.tildacdn.com/')!==-1) return true;
    return /\.(?:jpe?g|png|webp|gif|avif)(?:[?#]|$)/i.test(u);
  }

  function addUrl(list, seen, raw){
    if(!raw) return;
    String(raw).split(',').forEach(function(part){
      var token=part.trim().split(/\s+/)[0];
      var u=cleanUrl(token);
  ...
  function extractImages(doc){
    ...
    var rec=doc.getElementById(LEGACY_GALLERY_REC);
    var scope=rec || doc;

    var attrs=[
      'data-original','data-src','data-lazy-src','data-img-zoom-url',
      'data-bg','data-bg-img','data-original-src','src','srcset'
    ];

    scope.querySelectorAll('*').forEach(function(el){
      attrs.forEach(function(a){
        if(el.hasAttribute && el.hasAttribute(a)) addUrl(list,seen,el.getAttribute(a));
      });
      var style=el.getAttribute && el.getAttribute('style');
      if(style){
        var m;
        var rx=/url\((['"]?)(.*?)\1\)/g;
        while((m=rx.exec(style))) addUrl(list,seen,m[2]);
      }
    });

    /* Tilda sometimes stores lazy image URLs only in serialized markup/scripts. */
    var html=scope.outerHTML || '';
    var rxAbs=/https?:\\?\/\\?\/static\.tildacdn\.com\\?\/[^"'<>\s)]+/g;
    var matches=html.match(rxAbs)||[];
    matches.forEach(function(u){ addUrl(list,seen,u.replace(/\\\//g,'/')); });

    /* Prefer real product photos over tiny technical placeholders. */
    return list.filter(function(u){
      return !/(blank\.gif|empty\.png|pixel|favicon|icon-)/i.test(u);
    });
  }

  function applyImages(imgs){
    if(!imgs || imgs.length<2) return false;
    var api=window.FilinMasterProductV3;
    if(!api || !api.profiles || !api.profiles[SLUG]) return false;
    var p=api.profiles[SLUG];
    p.overview=p.overview||{};
    var existing=Array.isArray(p.overview.galleryImages)?p.overview.galleryImages:[];

    /* Keep the current hero/product image first when legacy extraction omits it. */
    var merged=[];
    var seen=Object.create(null);
    function push(u){
      u=cleanUrl(u); if(!u) return;
      var k=u.replace(/[?#].*$/,'');
      if(seen[k]) return;
      seen[k]=1; merged.push(u);
    }
    ...
    console.info('[Master Product V3 Profile] SIRIUS COMPLETE GALLERY RESTORED',{
  ...
  function waitAndApply(imgs,tries){
    tries=tries||0;
    if(applyImages(imgs)) return;
    if(tries>=50) return;
    setTimeout(function(){ waitAndApply(imgs,tries+1); },100);
  }

  function start(){
    if(started) return;
    started=true;
    fetch(LEGACY_URL,{credentials:'same-origin',cache:'no-store'})
      .then(function(r){ if(!r.ok) throw new Error('HTTP '+r.status); return r.text(); })
      .then(function(html){
        var doc=new DOMParser().parseFromString(html,'text/html');
        var imgs=extractImages(doc);
        console.info('[Master Product V3 Profile] SIRIUS LEGACY GALLERY FOUND',{images:imgs.length, galleryImages:imgs.slice()});
        waitAndApply(imgs,0);
      })
      .catch(function(err){
        console.warn('[Master Product V3 Profile] SIRIUS LEGACY GALLERY SYNC FAILED',err);
      });
  }

  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',function(){setTimeout(start,80);},{once:true});
